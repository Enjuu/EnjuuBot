...
public class cmdBest implements Command{
	...
	public void action(String[] args, MessageReceivedEvent event)
		...
		if(args.length == 0) {
			EmbedBuilder error = Static.CREATE_NORMAL_ERROR("404", "Wrong Arguments", "(ಥ﹃ಥ)", "-best <Name/ID>");
			event.getTextChannel().sendMessage(error.build()).queue();;
		}else if(args.length > 1) {
			EmbedBuilder error = Static.CREATE_NORMAL_ERROR("404", "Too much Arguments?", "ಠ_ಠ", "-best <Name/ID>");
			event.getTextChannel().sendMessage(error.build()).queue();;
		}else{
			try{
		        ...
		        URL u = null;
				try {
					u = new URL(Config.getString("apiprotocol")+"://"+Config.getString("api")+"/api/v1/users/scores/best?id="+args[0]);
		        ...
		        try{
				 ...
				 event.getTextChannel().sendMessage(ebload.build()).queue( message -> message.delete().queueAfter(3, TimeUnit.SECONDS) );
						...
						try {
			                ...
			                //Display
		    				eb.addField("Best "+Config.getString("name")+ " Score of " + User.ID_TO_USER(Long.parseLong(args[0])), "", false);
		    				...
		    				event.getTextChannel().sendMessage(eb.build()).queue();;
			    				...
			    				eb.addBlankField(true);
			    				eb.addField("-best <Name/ID>", "", false);
			    				event.getTextChannel().sendMessage(eb.build()).queue();;
			                }
			            ...
			            try (InputStream stream = urlConnection.getInputStream();
			              ...
			              getter = result.toString();
							...
							try {
				                ...
				                //Display
			    				eb.addField("Best "+Config.getString("name")+ " Score of " + args[0], "", false);
			    				eb.addBlankField(true);
			    				eb.addField("Beatmap:", song_name + "("+id.toString()+")", false);
			    				eb.addField("Score:", score.toString(), false);
			    				eb.addField("Max Combo:", max_combo.toString(), false);
