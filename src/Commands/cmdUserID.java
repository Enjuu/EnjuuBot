...
public class cmdUserID implements Command{
	...
	public void action(String[] args, MessageReceivedEvent event)
		...
		if(args.length == 0) {
			...
			EmbedBuilder error = Static.CREATE_NORMAL_ERROR("404", "Wrong Arguments", "(ಥ﹃ಥ)", "-userid <UserID>");
			event.getTextChannel().sendMessage(error.build()).queue();;
		}else{
			if(args.length > 1) {
				
				EmbedBuilder error = Static.CREATE_NORMAL_ERROR("404", "Too much Arguments?", "ಠ_ಠ", "-userid <UserID>");
				event.getTextChannel().sendMessage(error.build()).queue();;
