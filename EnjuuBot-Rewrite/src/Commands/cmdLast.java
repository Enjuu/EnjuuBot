...
public class cmdLast implements Command{
	...
	public void action(String[] args, MessageReceivedEvent event)
		...
		if(args.length == 0) {
			ErrorBuilder error = new ErrorBuilder(event, "The Command Usage is wrong.", "last <Name/ID>");
			error.sendError();
			return;
		}else if(args.length > 1) {
			ErrorBuilder error = new ErrorBuilder(event, "You give the Bot too much Arguments.", "last <Name/ID>");
			error.sendError();
			return;
		}else{
			if(isNumeric(args[0])) {
				...
				try {
					...
					ErrorBuilder error = new ErrorBuilder(event, "This User don't exist.", "last <Name/ID>");
					error.sendError();
					return;
				}
				Score s = null;
				try {
					...
					ErrorBuilder error = new ErrorBuilder(event, "This User don't have a best Score.", "last <Name/ID>");
					error.sendError();
					return;
				}
